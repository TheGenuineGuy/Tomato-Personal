...
interface StatDao {
    ...
    @Query(
        "SELECT " +
                "    CAST(COALESCE(SUM(focusTimeQ1), 0) / :n AS INTEGER) AS focusTimeQ1," +
                "    CAST(COALESCE(SUM(focusTimeQ2), 0) / :n AS INTEGER) AS focusTimeQ2," +
                "    CAST(COALESCE(SUM(focusTimeQ3), 0) / :n AS INTEGER) AS focusTimeQ3," +
                "    CAST(COALESCE(SUM(focusTimeQ4), 0) / :n AS INTEGER) AS focusTimeQ4," +
                "    CAST(COALESCE(SUM(breakTime), 0) / :n AS INTEGER) AS breakTime " +
                "FROM stat " +
                "WHERE date >= :startDate"
    )
    fun getLastNDaysAvgStats(n: Int, startDate: LocalDate): Flow<StatTime?>
